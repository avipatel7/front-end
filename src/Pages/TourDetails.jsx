...
const TourDetails = () => {
  ...
  // Fetch weather data using lat/lon coordinates
  useEffect(() => {
    const fetchWeatherData = async () => {
      try {
        // Extract the first part of the city before the comma
        const cityName = tour.city.split(",")[0].trim();

        const response = await axios.get(
          `${BASE_URL}/weather/${cityName}` // Replace with the actual API endpoint
        );
        setWeatherData(response.data); // Store the current weather data
